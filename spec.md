...
| ----------------- | ------- | -------------------------------------------------- |
| `id`              | INTEGER | PK                                                 |
| `slug`            | TEXT    | `swings`, `lat_pulldown`, `bench`, etc.  |
| `display_name`    | TEXT    |                                                    |
...
### 8.2 `user_settings` (singleton row)

| Column                  | Type    |
| ----------------------- | ------- |
| `onboarded_at`          | INTEGER | epoch millis               |
...
| -------------- | ------- | -------------------------------------- |
| `id`           | INTEGER | PK                                     |
| `date`         | TEXT    | ISO date (local), unique-per-day       |
| `split`        | TEXT    | `A` / `B` / `C`                        |
...
| `target_reps`  | INTEGER | nullable for KB (KB reps are fixed)                          |
| `weight_kg`    | REAL    | working weight or KB weight                                  |
| `status`       | TEXT    | `completed` / `failed`                                       |

### 8.5 `in_progress_session` (singleton)

Holds the unfinalized button state if the user is mid-workout. Cleared on commit. Schema mirrors `set_entry` plus a `set_state` enum (`pending` / `completed` / `failed`). On commit, pending rows would block — but in practice the UI does not allow commit until everything is non-pending.
